...
- [ ] Eligibility matcher produces accurate scores
...
- [ ] Resume generator produces ATS-passing PDFs
- [ ] Keyword injection is natural, not stuffed
- [ ] All 6 adapter types work (Greenhouse, Lever, Workday, Naukri, Google Forms, Universal)
- [ ] Session cookies correctly maintain login state
- [ ] CAPTCHA detection works and notifies correctly
- [ ] Email classifier accurately categorises all email types
...
- [ ] Telegram notifications fire for all event types
- [ ] Daily Google Sheet report generates automatically

### Security Checklist
- [ ] No API keys or passwords in Git history
- [ ] All sensitive MongoDB fields encrypted
- [ ] HTTPS on all deployed services
- [ ] Environment variables on Railway and Vercel, not in code
- [ ] playwright-stealth active on all automation sessions
- [ ] Human simulation delays on all form interactions
- [ ] Rate limiting on all external requests
- [ ] Audit log capturing all system actions
- [ ] Session warmup protocol for every new browser session

### Code Quality Checklist
- [ ] Every function has a docstring
- [ ] Every module has error handling
- [ ] Every database operation has a try-catch
- [ ] Logging throughout (not just print statements)
...
|---|---|---|
| Phase 1: Foundations | Weeks 1-4 | Python, Git, MongoDB CRUD, HTTP, async, scraping, embeddings |
| Phase 2: Core Modules | Weeks 5-8 | Resume generator, form automation, orchestrator, debate system |
| Phase 3: Integration | Weeks 9-12 | Company intelligence, email monitor, dashboard, deployment |
| Phase 4: Advanced | Weeks 13-16 | Security, feedback loop, fine-tuning, portfolio documentation |

**Total: 16 weeks (4 months) at 3-4 hours per day**
...
**System Architecture:**
5. Describe the complete data flow from a WhatsApp paste to a submitted application.
6. Why is one MongoDB cluster sufficient to replace ChromaDB, Redis, and a graph database?
7. How does the Orchestrator Agent decide which tools to use?
8. What is the ReAct pattern and why is it used for the orchestrator?

**AI Layer:**
9. What is an embedding and why does mathematical similarity of number arrays correspond to semantic similarity of text?
10. What is the difference between vector search and GraphRAG and when does each produce better results?
11. Why are there 6 debate agents rather than 1 agent doing all the analysis?
12. How does the Technical Expert agent get real knowledge beyond its training data?

**Security and Production:**
13. List 5 signals that distinguish an automated browser from a human browser.
14. Why must sensitive MongoDB fields be encrypted at the field level, not just at the database level?
15. What is LoRA fine-tuning and why does it not require as much compute as full fine-tuning?
16. Why do you keep resume version history and what data does each version store?
...
*Last updated: Day 1 — Version 1.0*